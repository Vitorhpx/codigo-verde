...
const App = () => {
  ...
  const [activePage, setActivePage] = React.useState<Pages>(Pages.login);
    ...
    <GlobalThemeProvider>
      <SnackbarProvider maxSnack={1}>
        {activePage !== Pages.login && (
          <Header
            onBackClick={getBackClick(activePage, setActivePage)}
            text={getHeaderText(activePage)}
          />
        )}

        {activePage === Pages.login && (
          <SignIn onLogin={() => setActivePage(Pages.main)} />
        )}
        ...
        {activePage === Pages.checkout && (
          <ProductCheckout
            products={products}
            onNextClick={() => setActivePage(Pages.places)}
          />
        )}
        {activePage === Pages.places && (
          <PlacesSelection places={placesMock} onNextClick={handleCheckout} />
        )}
        {activePage === Pages.thanks && (
          <Thanks onNextClick={() => setActivePage(Pages.main)} />
        )}
      </SnackbarProvider>
    </GlobalThemeProvider>
  );
